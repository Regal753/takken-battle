...
for (const source of data.SOURCES) {
  exactKeys(source, ["id", "publisher", "title", "url", "checkedAt"], `source ${source.id}`);
...
for (const card of data.FRESHNESS_CARDS) {
  exactKeys(card, ["id", "category", "title", "status", "reviewedAt", "effectiveOn", "expiresOn", "sourceIds", "answerPolicy", "checkpoint"], `card ${card.id}`);
