...
assert.match(index, /id="todayCommandOfficialLink"/);
assert.match(index, /RETIO公式20問を開く/);
assert.match(index, /① 宅建業法を固める/);
assert.match(index, /② 法令・税その他へ進む/);
assert.match(index, /③ 全分野を混ぜる/);
assert.match(index, /styles\.css\?v=20260729-structure-v9/);
assert.match(index, /app\.js\?v=20260729-structure-v9/);
...
assert.match(pagesWorkflow, /20260729-structure-v9/);
