(function (root, factory) {
  ...
  const MOCK_STABILITY_POLICY = Object.freeze({
    ...
    // v51 readiness is earned only on the three long-form 2026-law forms.
    // Earlier A/B/C records remain in saved history, but are intentionally
    // excluded so their shorter, older construction cannot certify readiness.
    eligibleFormIds: Object.freeze(["reiwa-form-a", "reiwa-form-b", "reiwa-form-c"]),
  ...
  function buildDailyPlan(todayKey, minutes, profile, officialTransfer, textbookFirstPass) {
    ...
    if (theme.mode === "mock") {
      ...
      if (!textbookFirstPass?.valid || textbookFirstPass.remainingUnits > 0) return { availableMinutes: minutes, weekday: weekdayFor(todayKey), mode: "required-full-mock", requiredMeasurement: "internal-mock", businessKnock: null, theme: { ...theme, minutes: profile.minutes, requiredTimedMinutes: profile.minutes, fitsAvailableMinutes: minutes >= profile.minutes }, choices: [full], review: null, note: "全45単元の高速一周前、または進捗未確認です。短縮復習は選べません。令和実戦A・B・Cの本試験形式測定で現在地を確認してください。" };
