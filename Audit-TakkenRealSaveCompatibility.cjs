...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() =>
      (document.querySelector("#saveProtectionStatus")?.textContent || "").includes("v6")
    );
    ...
    const readback = await page.evaluate((id) => {
      ...
      return {
        ...
        upgradeRaw: localStorage.getItem(`${id}-before-upgrade-v0-to-v6`) || "",
    ...
    assert.equal(readback.state.stateSchemaVersion, 6);
