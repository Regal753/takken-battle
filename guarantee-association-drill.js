...
(() => {
  ...
  const Q = [
    ...
    make("ga005", "単一選択", "新たに保証協会の社員となった宅建業者が、分担金を納付すべき期限はいつか。", 0, [["社員となろうとする日まで", "社員となろうとする者は、その日までに分担金を納付する。", `${L("64条の9")}`, A], ["社員となった日から1週間以内", "1週間は協会の供託期限であり、社員の加入前納付期限ではない。", `${L("64条の9")}`, A], ["社員となった日から2週間以内", "2週間は新設事務所分の納付期限である。", `${L("64条の9")}`, A], ["免許日から1か月以内", "この期限は定められていない。", `${L("64条の9")}`, A]], "加入は『なる前』に納付。", "加入後2週間と新設事務所2週間を取り違える。", "加入=前払い。", ["timing"]),
    ...
    make("ga009", "単一選択", "保証協会の社員と取引した者が弁済業務保証金から還付を受けられる者として正しいものはどれか。", 1, [["宅建業者である取引相手だけ", "宅建業者は還付請求権者から除かれる。", `${L("64条の8")}`, A], ["宅建業者でない取引相手で、社員となる前の取引による債権者も含む", "宅建業者以外の者で、社員となる前の取引から生じた債権者も含まれる。", `${L("64条の8")}`, A], ["社員となった後の取引に限る", "加入前の取引による債権者も含む。", `${L("64条の8")}`, A], ["保証協会の社員だけ", "社員自身のための還付制度ではない。", `${L("64条の8")}`, A]], "消費者保護。相手が宅建業者なら除外、加入前債権も入る。", "加入前取引は対象外と思い込む。", "非業者＋前の債権も可。", ["subject", "transaction-type"]),
    make("ga010", "計算問題", "主たる事務所1、従たる事務所2の保証協会社員について、取引相手が還付を受けられる上限はいくらか。", 2, [["120万円", "分担金額（60+30×2）を上限とするわけではない。", `${L("64条の8")}`, A], ["1,500万円", "従たる事務所1か所分が不足する。", `${L("25条1項")}`, A], ["2,000万円", "営業保証金を供託していたなら必要な額は1,000万円+500万円×2。これが還付上限。", `${L("64条の8")}`, A], ["無制限", "還付には営業保証金相当額という上限がある。", `${L("64条の8")}`, A]], "還付上限は60・30ではなく、営業保証金なら必要だった1000・500。", "分担金額がそのまま被害者上限だと誤解する。", "被害者上限=営業保証金換算。", ["number", "calculation"]),
    ...
    make("ga012", "単一選択", "弁済業務保証金の還付請求に必要な保証協会の認証申出書は何通作成するか。", 2, [["1通", "申出書は3通作成する。", `${G("26条の5")}`, R], ["2通", "申出書は3通作成する。", `${G("26条の5")}`, R], ["3通", "認証の申出書は3通。", `${G("26条の5")}`, R], ["4通", "4通ではない。", `${G("26条の5")}`, R]], "認証申出書は3通。数字単体で固定する。", "供託書類一般の部数で推測する。", "認証=3通。", ["number", "procedure"]),
    ...
    make("ga016", "事例問題", "保証協会の社員資格を失った宅建業者が営業を続けるため、営業保証金を供託すべき期限はいつか。", 2, [["資格喪失前日まで", "資格喪失前に営業保証金の供託義務が生じるわけではない。", `${L("64条の15")}`, A], ["資格喪失から2週間以内", "2週間ではなく1週間。", `${L("64条の15")}`, A], ["資格喪失から1週間以内", "社員資格を失った者は1週間以内に営業保証金を供託する。", `${L("64条の15")}`, A], ["資格喪失から1か月以内", "1か月ではない。", `${L("64条の15")}`, A]], "資格喪失→営業保証金1週。", "新設事務所2週と混同する。", "資格喪失=1週で営業保証金。", ["timing", "effect"]),
    make("ga017", "単一選択", "保証協会が指定を取り消され、又は解散した場合、元社員が営業保証金を供託すべき期限はどれか。", 1, [["社員資格を失った日から1週間以内", "通常の社員資格喪失は1週間だが、保証協会の指定取消し・解散には別の起算点がある。", `${L("64条の15")}`, A], ["その旨の公示の日から2週間以内", "指定取消し・解散時は公示の日から2週間以内に営業保証金を供託する。", `${L("64条の23")}`, A], ["その旨の公示の日から1か月以内", "公示日から1か月ではなく2週間以内である。", `${L("64条の23")}`, A], ["6か月を下らない期間内", "6か月以上は社員資格喪失時の認証申出公告の期間である。", `${L("64条の11")}`, A]], "通常の資格喪失は喪失日から1週、協会の取消し・解散は公示日から2週。", "資格喪失の原因を読まず、すべて1週にする。", "通常喪失1週／取消し・解散の公示後2週。", ["timing", "principle-exception"]),
    ...
    make("ga020", "単一選択", "次の期限対応のうち、すべて正しく対応しているものはどれか。", 2, [["加入=加入後2週／新設=1週／協会受領=2週／資格喪失=1週", "加入は加入前、新設は2週、協会受領は1週なので誤り。", `${L("64条の9")}・${L("64条の7")}・${L("64条の15")}`, A], ["加入=加入前／新設=2週／協会受領=1週／資格喪失=2週", "資格喪失後の営業保証金は1週であり、最後が誤り。", `${L("64条の9")}・${L("64条の7")}・${L("64条の15")}`, A], ["加入=加入前／新設=2週／協会受領=1週／資格喪失=1週", "加入前・新設2週・協会受領1週・資格喪失1週の対応が正しい。", `${L("64条の9")}・${L("64条の7")}・${L("64条の15")}`, A], ["加入=加入前／新設=1か月／協会受領=1週／資格喪失=1週", "新設事務所分は1か月ではなく2週間以内。", `${L("64条の9")}・${L("64条の7")}・${L("64条の15")}`, A]], "期限を孤立暗記せず、当事者と並べて比較する。", "資格喪失後の営業保証金を2週とする。", "加入前／新設2週／協会1週／喪失1週。", ["timing", "procedure", "counterparty"])
  ];
  ...
  function exactSource(questionId, index) {
    ...
    if (questionId === "ga017") return index === 0 ? [L("64条の15"), A] : index === 3 ? [L("64条の11"), A] : [L("64条の23"), A];
  ...
  const QUESTIONS = freeze(Q.map((question) => {
    ...
    return freeze({ ...question, unitLabel: UNITS[0].label, formatKey: formatKeys[question.format], sourceFacts, choiceExplanations, statementExplanations: choiceExplanations, sourceUrl: sourceFacts[0].sourceUrl, sourceUrls: freeze([...new Set(sourceFacts.flatMap((item) => item.sourceUrls))]) });
  }));
