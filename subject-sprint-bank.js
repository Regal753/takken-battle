...
(function attachSubjectSprintBank(root, factory) {
  ...
  const api = factory(root, supplement);
  if (typeof module === "object" && module.exports) module.exports = api;
  root.TAKKEN_SUBJECT_SPRINT_BANK = api;
  if (root.window && root.window !== root) root.window.TAKKEN_SUBJECT_SPRINT_BANK = api;
})(typeof globalThis !== "undefined" ? globalThis : this, function createSubjectSprintBank(runtime, requiredSupplement) {
  ...
  if (!blueprint || !baseQuestions || !restrictionsSupplement) {
    throw new Error("subject sprint bank requires the blueprint, base questions and restrictions supplement");
  }
  ...
  const sourceGroups = Object.freeze([
    ...
    ["restrictions", "law", ["l001", "l002", "l003", "l004", "l005", "l006", "l007", "l008", "l009", "l010", "l011", "l012", "l013", "l014", "l015", "l016", "l101", "l102", "rs001", "rs002", "rs003", "rs004", "rs005", "rs006", "rs007", "rs008", "rs009", "rs010", "rs011", "rs012", "rs013", "rs014", "rs015", "rs016", "rs017", "rs018", "rs019", "rs020", "rs021", "rs022"]],
  ...
  const questions = definitions.map(([id, baseId, sectionId, tag, diagnosticTags, variantOffset]) => {
    ...
    return Object.freeze({id,masteryId:id,sourceQuestionId:baseId,sourceAnchor:clean(source.sourceAnchor||source.sourceRef),sectionId,tag,diagnosticTags,variantOffset,format:source.format,text:source.text,choices:source.choices,answer:source.answer,explain:source.explain,trap:source.trap,memoryRule:source.memoryRule,groundingFrame:source.groundingFrame||null,choiceExplanations:source.choiceExplanations,statementExplanations:source.choiceExplanations,sourceFacts:baseFacts(source),sourceRef:source.sourceRef,sourceLocator:source.sourceLocator,sourceUrl:source.sourceUrl,sourceUrls:Object.freeze((Array.isArray(source.sourceUrls)?source.sourceUrls:[source.sourceUrl]).map(clean).filter(Boolean)),legalBaseline:source.legalBaseline,verifiedAt:source.verifiedAt});
  });
  ...
  function diversify(questionIds, recentQuestionIds = []) {
    ...
    while (pending.length) {
      ...
      pending.forEach(({ question, index }, candidateIndex) => {
        ...
        // A safe different-law candidate always wins; among equally safe
        // candidates retain the caller's mastery-priority ranking.
        const score = [anchorPenalty, index];
