...
// Independent snapshot of all 25 browser commands that preceded sharding.
const required = [
...
(async () => {
  assert.equal(runner.EXPECTED_AUDIT_COUNT, 25);
  ...
  console.log(JSON.stringify({ status: "ok", audits: 25, shards: shards.map(shard => shard.length), missing: 0, duplicates: 0, invalidShardRejected: true, emptyRejected: true, failureExitPreserved: true, matrixFailFast: false, timeoutMinutes: 15 }));
