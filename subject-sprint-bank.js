...
(function attachSubjectSprintBank(root, factory) {
  ...
  if (restrictionsSupplement.LEGAL_BASELINE !== "2026-04-01" || restrictionsSupplement.QUESTIONS.length !== 14) {
  ...
  const sourceGroups = Object.freeze([
    ...
    ["restrictions", "law", ["l001", "l002", "l003", "l004", "l005", "l006", "l007", "l008", "l009", "l010", "l011", "l012", "l013", "l014", "l015", "l016", "l101", "l102", "rs001", "rs002", "rs003", "rs004", "rs005", "rs006", "rs007", "rs008", "rs009", "rs010", "rs011", "rs012", "rs013", "rs014"]],
  ...
  const topicAliases = Object.freeze({
    ...
    rs001:["開発工事の完了検査・公告",["development-permit","completion-notice"]],rs002:["市街化調整区域の建築許可",["urbanization-control-area","building-permit"]],rs003:["令和7年建築確認対象見直し",["building-confirmation","current-law"]],rs004:["木造戸建ての大規模修繕",["building-confirmation","major-renovation","current-law"]],rs005:["国土法の国籍等届出事項",["national-land","notification-items","current-law"]],rs006:["事後届出後の勧告・公表",["national-land","recommendation"]],rs007:["市街化区域内の農地転用届出",["agricultural-land","conversion","urbanization-area"]],rs008:["農地取得の効力・相続・一時転用",["agricultural-land","inheritance","temporary-conversion"]],rs009:["換地処分公告後の権利移転",["land-readjustment","replot-disposition"]],rs010:["減歩・清算金",["land-readjustment","land-reduction","settlement-money"]],rs011:["盛土規制法の対象・区域",["embankment","regulated-area"]],rs012:["盛土許可申請前後の義務",["embankment","permit-process"]],rs013:["遺跡発見時の届出",["cultural-property","discovery-notice"]],rs014:["道路占用許可と道路使用許可",["road-occupation","road-use-permit"]],
  ...
  const questions = definitions.map(([id, baseId, sectionId, tag, diagnosticTags, variantOffset]) => {
    ...
    return Object.freeze({id,masteryId:id,sourceQuestionId:baseId,sourceAnchor:clean(source.sourceAnchor||source.sourceRef),sectionId,tag,diagnosticTags,variantOffset,format:source.format,text:source.text,choices:source.choices,answer:source.answer,explain:source.explain,trap:source.trap,memoryRule:source.memoryRule,choiceExplanations:source.choiceExplanations,statementExplanations:source.choiceExplanations,sourceFacts:baseFacts(source),sourceRef:source.sourceRef,sourceLocator:source.sourceLocator,sourceUrl:source.sourceUrl,sourceUrls:Object.freeze((Array.isArray(source.sourceUrls)?source.sourceUrls:[source.sourceUrl]).map(clean).filter(Boolean)),legalBaseline:source.legalBaseline,verifiedAt:source.verifiedAt});
  });
