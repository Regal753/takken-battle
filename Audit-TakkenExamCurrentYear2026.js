...
assert.equal(data.REVIEWED_AT, "2026-09-10");
...
assert.match(data.SOURCE_BY_ID["retio-2026-exam-guide"].url, /^https:\/\/www\.retio\.or\.jp\/wp-content\/uploads\/2026\/05\/.+\.pdf$/);
...
assert.deepEqual(data.assessFreshness("law-baseline-2026-04-01", "2026-09-10"), { status: "current", current: true, failClosed: false, reason: null });
...
assert.deepEqual(data.assessFreshness("missing", "2026-09-10"), { status: "unknown", current: false, failClosed: true, reason: "invalid-or-missing-freshness-data" });
...
assert.deepEqual(data.assessFreshness("law-baseline-2026-04-01", "2026-09-26"), { status: "stale", current: false, failClosed: true, reason: "review-receipt-expired" });
const baseCard = data.FRESHNESS_CARD_BY_ID["law-baseline-2026-04-01"];
const refreshedCard = {
  ...baseCard,
  reviewedAt: "2026-09-22",
  checkedAt: "2026-09-22",
  reviewReceipt: { ...baseCard.reviewReceipt, checkedAt: "2026-09-22" }
};
assert.deepEqual(data.assessFreshness(refreshedCard, "2026-09-26"), { status: "unknown", current: false, failClosed: true, reason: "invalid-or-missing-review-receipt" });
...
assert.deepEqual(data.assessFreshness(changedHashCard, "2026-09-10"), { status: "unknown", current: false, failClosed: true, reason: "invalid-or-missing-review-receipt" });
const missingReceiptCard = { ...baseCard, reviewReceipt: null };
assert.deepEqual(data.assessFreshness(missingReceiptCard, "2026-09-10"), { status: "unknown", current: false, failClosed: true, reason: "invalid-or-missing-review-receipt" });
const missingSourceCard = { ...baseCard, sourceIds: ["untrusted-example"] };
assert.deepEqual(data.assessFreshness(missingSourceCard, "2026-09-10"), { status: "unknown", current: false, failClosed: true, reason: "missing-primary-source" });
const invalidDateCard = { ...baseCard, checkedAt: "2026-02-30" };
assert.deepEqual(data.assessFreshness(invalidDateCard, "2026-09-10"), { status: "unknown", current: false, failClosed: true, reason: "invalid-or-missing-freshness-data" });
const allCurrent = data.assessAllFreshness("2026-09-10");
...
const allStale = data.assessAllFreshness("2026-09-26");
