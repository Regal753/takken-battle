(function (root, factory) {
  ...
  const MOCK_STABILITY_POLICY = Object.freeze({
    ...
    // These are practice diagnostics, not independent or calibrated exam
    // evidence. Keep the historical class readable without upgrading its value.
    // A single authored form must not create a mandatory multi-form gate.
    eligibleFormIds: Object.freeze(["reiwa-form-a", "reiwa-form-b", "reiwa-form-c", "case-form-2026-a"]),
    eligibleEvidenceClasses: Object.freeze(["recombined-practice", "authored-case-practice", "independent-current-law"])
  });
  ...
  function currentLawGateFrom(input, todayKey) {
    ...
    const attemptAgeDays = recent.map((attempt) => daysBetween(attempt.dateKey, todayKey));
    const recentEnough = attemptAgeDays.length === 2 && attemptAgeDays.every((ageDays) => ageDays !== null && ageDays >= 0 && ageDays <= CURRENT_LAW_ATTEMPT_MAX_AGE_DAYS);
    ...
    return { suppliedCount: Array.isArray(raw.attempts) ? raw.attempts.length : 0, validAttemptCount: attempts.length, requiredAttempts: 2, recent, requiredClusters: [...CURRENT_LAW_CLUSTERS], coveredClusters: [...coverage], latestDayKey: latest ? latest.dateKey : "", ageDays, attemptAgeDays, maxAttemptAgeDays: CURRENT_LAW_ATTEMPT_MAX_AGE_DAYS, recentEnough, consecutive, covered, passed, status: attempts.length < 2 ? "unmeasured" : passed ? "passed" : "failed" };
  }
  ...
  function rotatingTheme(todayKey) { const weekday = weekdayFor(todayKey); const lookup = [{ key: "mock", label: "本試験形式", mode: "mock" }, { key: "tax-other", label: "税・価格・免除科目等", mode: "drill" }, { key: "restrictions", label: "法令上の制限", mode: "drill" }, { key: "rights", label: "権利関係", mode: "drill" }, { key: "tax-other", label: "税・価格・免除科目等", mode: "drill" }, { key: "restrictions", label: "法令上の制限", mode: "drill" }, { key: "rights", label: "権利関係", mode: "drill" }]; return weekday === null ? null : lookup[weekday]; }
  function finalStretchFrom(todayKey) {
    const daysToExam = daysBetween(todayKey, EXAM_DAY_KEY);
    const active = todayKey > FIRST_PASS_DEADLINE_KEY && daysToExam > 0;
    const phase = !active ? "foundation" : todayKey <= "2026-09-15" ? "coverage" : daysToExam > 7 ? "repair" : "final-week";
    const label = { foundation: "基礎一周", coverage: "全科目の未接触を回収", repair: "測定して弱点を補修", "final-week": "誤答・数字・例外を最終確認" }[phase];
    const note = phase === "coverage"
      ? "9/15までに法令・税・その他の未接触を一周。問題から入り、分からなかった箇所だけ教科書へ戻ります。"
      : phase === "repair"
        ? "週1回は公式未見を本番時間で測定。誤答は理由を確認し、翌日以降に答えを隠して解き直します。"
        : phase === "final-week"
          ? "新しい教材を増やさず、残る誤答・数字・例外と受験票を確認。前日は軽い復習と持ち物・移動の準備を優先します。"
          : "基礎を一周し、科目ごとの未接触を減らします。";
    return { active, phase, label, note, daysToExam };
  }
  function buildDailyPlan(todayKey, minutes, profile, officialTransfer, textbookFirstPass) {
    ...
    const finalStretch = finalStretchFrom(todayKey);
    if (theme.mode === "mock") {
      const full = { key: "full-mock", label: `本試験${profile.questions}問・${profile.minutes}分（今日はこれだけ）`, minutes: profile.minutes, primary: true };
      if (!finalStretch.active && (!textbookFirstPass?.valid || textbookFirstPass.remainingUnits > 0)) return { availableMinutes: minutes, weekday: weekdayFor(todayKey), mode: "required-full-mock", requiredMeasurement: "internal-mock", businessKnock: null, theme: { ...theme, minutes: profile.minutes, requiredTimedMinutes: profile.minutes, fitsAvailableMinutes: minutes >= profile.minutes }, choices: [full], review: null, note: "全45単元の高速一周前、または進捗未確認です。短縮復習は選べません。事例実戦Aで不足論点を診断してください。内部演習の得点は合格証拠には使わず、公式未見問題の測定と区別します。" };
  ...
  function calculatePassReadiness(options = {}) {
    ...
    const timed50 = { mock: { ...mockHistory, requiredForReadiness: false, evidenceUse: "practice-diagnostics" }, official: officialHistory, officialTransfer, baseStable: officialTransfer.passed, currentLawGatePassed: currentLawGate.passed, stable: currentLawGate.passed && officialTransfer.passed, status: currentLawGate.passed && officialTransfer.passed ? "stable" : currentLawGate.status === "unmeasured" || officialTransfer.status === "unmeasured" ? "unmeasured" : "unstable" };
