...
(() => {
  ...
  const BUSINESS_HARD_QUESTIONS = Object.freeze(
    (Array.isArray(BUSINESS_HARD_BANK?.QUESTIONS) ? BUSINESS_HARD_BANK.QUESTIONS : [])
      .map(normalizeFullScoreQuestion).filter(Boolean)
  );
  const BUSINESS_HARD_QUESTION_BY_ID = Object.freeze(Object.fromEntries(
    BUSINESS_HARD_QUESTIONS.map((item) => [item.id, item])
  ));
  const BUSINESS_HARD_QUESTION_IDS = Object.freeze(BUSINESS_HARD_QUESTIONS.map((item) => item.id));
  // Save resolution includes both generations; the 134-question mastery gate remains separate.
  const BUSINESS_DRILL_QUESTION_BY_ID = Object.freeze({
    ...BUSINESS_FULLSCORE_QUESTION_BY_ID, ...BUSINESS_HARD_QUESTION_BY_ID
  });
  ...
  const BUSINESS_HARD_BANK_READY = Boolean(
    BUSINESS_FULLSCORE_BANK_READY && BUSINESS_HARD_BANK?.LEGAL_BASELINE === "2026-04-01" &&
    BUSINESS_HARD_QUESTIONS.length === BUSINESS_HARD_EXPECTED_QUESTIONS &&
  ...
  const BUSINESS_KNOCK_READY = BUSINESS_HARD_BANK_READY && Boolean(BUSINESS_KNOCK?.plan);
  ...
  const REIWA_FORMS = Array.isArray(REIWA_BANK?.forms) ? REIWA_BANK.forms.map((form) => ({
    ...
    evidenceClass: "recombined-practice",
    reiwaOriginal: true
  })) : [];
  const CASE_FORMS = (window.TAKKEN_CASE_EXAM_BANK?.forms || []).map(form => ({
    id: form.id, label: form.label, shortLabel: form.shortLabel,
    ids: form.questions.map(question => question.id),
    evidenceClass: form.evidenceClass, authoredCase: true
  }));
  const CASE_QUESTIONS = Object.fromEntries((window.TAKKEN_CASE_EXAM_BANK?.forms || [])
    .flatMap(form => form.questions.map(question => [question.id, question])));
  ...
  const ALL_MOCK_FORMS = [...(EXAM_BLUEPRINT?.mockForms || []), ...REIWA_FORMS, ...CASE_FORMS];
  const idToChapter = new Map();
  CHAPTERS.forEach((chapter, chapterIndex) => {
    chapter.ids.forEach((id) => idToChapter.set(id, { ...chapter, chapterIndex }));
  });
  [...REIWA_FORMS, ...CASE_FORMS].forEach((form, formIndex) => {
    (EXAM_BLUEPRINT?.sections || []).forEach((section, sectionIndex) => {
      const ids = form.ids.filter((id) => QUESTIONS[id]?.sectionId === section.id);
      const chapter = {
        id: `${form.id}-${section.id}`,
        label: `${form.authoredCase ? "事例実戦" : "複合練習"} / ${section.label}`,
  ...
  const elements = {
    ...
    businessKnockDifficulty: $("#businessKnockDifficulty"),
    businessKnockStats: $("#businessKnockStats"),
    businessKnockScope: $("#businessKnockScope"),
  ...
  function practicalQuestionSet(bankId = LEGACY_PRACTICAL_BANK_ID) {
    // The canonical hard IDs survive an asset-load failure. Rendering stops,
    // but a successful reload can resume the exact question and selected answer.
  ...
  function normalizePracticalDrillState(input) {
    ...
    if ([BUSINESS_FULLSCORE_BANK_ID, GUARANTEE_SPECIAL_BANK_ID, SUBJECT_SPRINT_BANK_ID].includes(bankId) &&
      ...
      Object.entries(input.presentationOverrides).forEach(([id, key]) => {
        if (!practicalQuestionFor(id, bankId) && !practicalQuestionSet(bankId).has(id)) return;
  ...
  function activeResumeLabel(active = activeLearningSession()) {
    if (active?.kind === "practical" && state.practicalDrill?.bankId === GUARANTEE_SPECIAL_BANK_ID) {
      return "保証協会特訓を保存位置から再開";
    }
    return `${active?.label || "進行中のセット"}を再開`;
  }
  ...
  function mockFormShortLabel(form = currentMockForm()) {
    if (!form) return "模試";
    if (form.authoredCase) return form.shortLabel;
  ...
  function latestMockAttempt() {
    return [...(state.mockHistory || [])]
      .filter((item) => {
        ...
        return form?.authoredCase && item.examProfile === state.examProfile;
      })
      .sort((left, right) =>
  ...
  function passPhaseFor(date = todayKey()) {
    if (!foundationCoverageComplete()) {
      ...
      return {
        id: "foundation",
        title: "8/31まで高速一周",
        text: `業法20問は維持し、税その他→法令→権利の順で未接触単元を潰す。事例実戦${examProfileSummary()}は弱点診断に使い、RETIO公式未見は保全する。`
      };
    }
    if (date <= "2026-08-31") {
      return {
    ...
    if (date <= "2026-09-30") {
      return {
        id: "september",
        title: "本試験演習",
        text: "本試験の測定は未見の公式過去問で行い、別3試験回・別3日で得点を確認する。事例実戦と複合練習は弱点診断用で、合格の証拠には算入しない。"
      };
    }
    if (date <= "2026-10-11") {
      return {
  ...
  function passReadinessSnapshot() {
    ...
    return PASS_READINESS.calculatePassReadiness({
      ...
      // RETIO過去問は当時法の採点。現行法の確認は専用ゲートで分ける。
      officialHistory: []
    });
  }
  ...
  function nextMockFormId() {
    const candidateForms = CASE_FORMS;
  ...
  function runPassCommandAction(button) {
    ...
    } else if (action === "guarantee-special") {
  ...
  function renderPassReadinessTodayCommand(mission) {
    ...
    let secondary = null;
    if (active) {
      primary = { action: "resume", label: activeResumeLabel(active) };
      ...
      primary = officialMeasurementRequired
        ...
        : { action: "mock", label: `事例実戦A ${snapshot.examProfile.questions}問・${snapshot.examProfile.minutes}分を開始` };
      ...
      : sundayDay
        ? fullMeasurementRequired
          ? officialMeasurementRequired
            ...
            : `全45単元の一周が未完了です。今日は事例実戦Aを本試験時間で解き、誤答と未測定の論点を確認します。`
    ...
    elements.missionBattleStatus.textContent = sundayDay
      ? fullMeasurementRequired
        ? officialMeasurementRequired ? "公式初見必須" : "事例演習"
    ...
    elements.missionOfficialLabel.textContent = sundayDay
      ? sundayMode === "full-mock"
        ? officialMeasurementRequired ? `公式未見${snapshot.examProfile.questions}問` : `事例実戦${snapshot.examProfile.questions}問`
        : "業法20＋弱点8"
      : theme.label;
  ...
  function renderPassReadinessCard(snapshot = passReadinessSnapshot()) {
    ...
    elements.passReadinessNote.textContent = snapshot.finalStretch?.active
      ...
      : `演習目標は${snapshot.targets.total}/${snapshot.targets.questions}。内部演習は弱点診断用です。未測定は弱点と決めつけず、公式初見の結果と現行法確認を分けて扱います。`;
    const mockEvidence = snapshot.timed50.mock;
    const officialEvidence = snapshot.timed50.officialTransfer;
    const formEvidenceLabel = "弱点診断用・合格証拠対象外";
    ...
    elements.passReadinessStatus.textContent = snapshot.timed50.stable && snapshot.currentYearFreshness.passed && snapshot.capacity.verified
      ? "公式初見の別3試験回・別3日、改正確認2日、当年資料、直近7日の学習時間を確認済み。合格を保証する表示ではありません。"
      : `内部演習 ${formEvidenceLabel}${excludedMockNote}・公式初見 ${officialEvidenceLabel}・改正確認 ${lawEvidenceLabel}・当年資料 ${snapshot.currentYearFreshness.passed ? "確認済" : "要再確認"}・学習時間 ${snapshot.capacity.verified ? "確認済" : `${snapshot.capacity.observedDays}/4日`}`;
  ...
  function diagnosticTagsForPracticalSelection(question, selected, uncertain = false) {
    ...
    const bank = BUSINESS_HARD_QUESTION_BY_ID[question.id] ? BUSINESS_HARD_BANK : BUSINESS_FULLSCORE_BANK;
    if (!uncertain && typeof bank?.diagnosticsForSelection === "function") {
      try {
        return normalizeBusinessTagList(
          bank.diagnosticsForSelection(question, selected)
        );
      } catch {
  ...
  function presentPracticalQuestion(question, bankId, presentationKey) {
    ...
    const bank = BUSINESS_HARD_QUESTION_BY_ID[question.id] ? BUSINESS_HARD_BANK : BUSINESS_FULLSCORE_BANK;
    if (typeof bank?.presentQuestion === "function") {
      try {
        const presented = normalizeFullScoreQuestion(
  ...
  function validPracticalDisplayBlock(block, sharedCasePremise = false) {
    return Boolean(
      block &&
      Array.isArray(block.premises) &&
      (block.premises.length || sharedCasePremise === true) &&
  ...
  function renderPracticalChoice(button, choice, index, block, sharedPremiseGroups = [], sharedCasePremise = false) {
  ...
  function renderPracticalDrill() {
    ...
    if (!question) {
      elements.practicalDrillPrompt.textContent = "このセットの問題を読み込めません。途中の問題順・解答は保持しています。再読込して再開してください。";
      elements.practicalDrillChoices.replaceChildren();
      elements.practicalDrillFeedback.hidden = true;
      elements.practicalDrillNextButton.disabled = true;
      return;
    }
    const attempt = drill.currentAttempt;
    ...
    elements.practicalDrillUnit.textContent = hardKnockSession && !attempt
      ? "高難度・事例判断（論点と根拠は解答後）"
      : question.unitPage
  ...
  function revealPracticalTarget(target) {
    if (!target) return;
    target.focus({ preventScroll: true });
    const viewportHeight = Math.max(1, Number(window.visualViewport?.height) || window.innerHeight || 1);
    const margin = Math.min(128, Math.max(64, Math.round(viewportHeight * 0.2)));
    const rect = target.getBoundingClientRect();
    if (rect.top < margin || rect.top > viewportHeight - margin || rect.bottom < margin) {
      const top = Math.max(0, window.scrollY + rect.top - margin);
      window.scrollTo(0, top);
    }
    window.requestAnimationFrame(() => {
      const settled = target.getBoundingClientRect();
      if (settled.top < 0 || settled.top >= viewportHeight) {
        window.scrollTo(0, Math.max(0, window.scrollY + settled.top - margin));
      }
    });
  }
  ...
  function focusCurrentPracticalInputWithMinimalScroll() {
    ...
    const hardCase = target === prompt;
    if (prompt && promptRect && (hardCase
      ? promptRect.top < 68 || promptRect.top >= window.innerHeight - 68
      : promptRect.bottom <= 0 || promptRect.top >= window.innerHeight)) {
  ...
  function businessPrimaryAction(summary = businessFullScoreSummary()) {
    const active = activeLearningSession();
    if (active) return { kind: "resume", label: activeResumeLabel(active) };
  ...
  function normalizeBusinessKnockPreset(input = state.practicalDrill?.knockPreset) {
    const difficulty = input?.difficulty === "basic" ? "basic" : "hard";
    ...
    const unitId = BUSINESS_FULLSCORE_UNITS.some((unit) => unit.id === input?.unitId &&
      (difficulty === "basic" || unit.id !== "business-book-05"))
      ? String(input.unitId)
    ...
    const lastPresentationOffset = Number.isInteger(input?.lastPresentationOffset) &&
      ...
      : null;
    return { difficulty, mode, size, unitId, lastPresentationOffset };
  }

  function businessKnockQuestions(preset) {
  ...
  function businessKnockHistory(preset) {
    return Object.fromEntries(businessKnockQuestions(preset)
      .filter((question) => state.practicalDrill?.history?.[question.id])
      .map((question) => [question.id, state.practicalDrill.history[question.id]]));
  }
  ...
  function guaranteeSpecialSummary(now = new Date()) {
    const history = guaranteeSpecialHistory();
    const states = { retry: 0, due: 0, untouched: 0, learning: 0, retained: 0, durable: 0 };
    const stateById = {};
    const currentDayKey = BUSINESS_MASTERY.dayKey(now);
    GUARANTEE_SPECIAL_QUESTIONS.forEach((question) => {
      const entry = history[question.id] || {};
      const rawState = BUSINESS_MASTERY.stateFor(entry, now);
      const learningState = rawState === "retry" && entry.retryNotBeforeKey > currentDayKey
        ? "learning"
        : rawState;
      stateById[question.id] = learningState;
      if (Object.hasOwn(states, learningState)) states[learningState] += 1;
    });
    ...
    const nextDueKey = GUARANTEE_SPECIAL_QUESTION_IDS
      .flatMap((id) => [
        BUSINESS_MASTERY.normalizeMasteryHistory(history[id] || {}).masteryDueKey,
        history[id]?.retryNotBeforeKey || ""
      ])
      .filter((key) => key && key > BUSINESS_MASTERY.dayKey(now))
      .sort()[0] || "";
    const overconfidentWrong = GUARANTEE_SPECIAL_QUESTION_IDS
      .reduce((sum, id) => sum + Math.max(0, Number(history[id]?.overconfidentWrong) || 0), 0);
    const hesitantCorrect = GUARANTEE_SPECIAL_QUESTION_IDS
      .reduce((sum, id) => sum + Math.max(0, Number(history[id]?.hesitantCorrect) || 0), 0);
    return {
      history,
      states,
      stateById,
      contacted,
      grounded,
      retained: states.retained + states.durable,
      review: states.retry + states.due,
      actionable: states.retry + states.due + states.untouched,
      overconfidentWrong,
      hesitantCorrect,
      nextDueKey
    };
  ...
  function guaranteeSpecialRankedQuestions(summary, strategy, seed) {
    const priority = { retry: 0, due: 1, untouched: 2, learning: 3, retained: 4, durable: 5 };
    const ranked = GUARANTEE_SPECIAL_QUESTIONS
      .map((question, index) => ({
        question,
        index,
        learningState: summary.stateById[question.id] || "untouched",
        attempts: Math.max(0, Number(summary.history[question.id]?.attempts) || 0)
      }))
      .filter((item) => strategy === "full" || ["retry", "due", "untouched"].includes(item.learningState))
      .sort((left, right) =>
        priority[left.learningState] - priority[right.learningState] ||
        left.attempts - right.attempts ||
        left.index - right.index
      );
    if (summary.contacted === 0) return ranked.map((item) => item.question);
    const mixed = [];
    for (let start = 0; start < ranked.length;) {
      let end = start + 1;
      while (end < ranked.length &&
        ranked[end].learningState === ranked[start].learningState &&
        ranked[end].attempts === ranked[start].attempts) end += 1;
      const bucket = ranked.slice(start, end);
      const order = BUSINESS_MASTERY.choiceOrder(
        `guarantee-${ranked[start].learningState}-${ranked[start].attempts}`,
        `${todayKey()}:${seed}`,
        bucket.length
      );
      mixed.push(...order.map((index) => bucket[index].question));
      start = end;
    }
    return mixed;
  }

  function guaranteeSpecialPlan(strategy = "smart", seed = "preview") {
    if (!GUARANTEE_SPECIAL_READY) return null;
    const summary = guaranteeSpecialSummary();
    const ranked = guaranteeSpecialRankedQuestions(summary, strategy, seed);
    const limit = strategy === "full"
      ? GUARANTEE_SPECIAL_EXPECTED_QUESTIONS
      : Math.min(10, ranked.length);
    return {
      strategy,
      summary,
      ids: ranked.slice(0, limit).map((question) => question.id),
      size: Math.min(limit, ranked.length)
    };
  }

  function businessKnockPlan(preset, seed = "preview") {
    if (!businessKnockReady(preset)) return null;
    ...
    return BUSINESS_KNOCK.plan({
      questions: businessKnockQuestions(normalized),
      history: businessKnockHistory(normalized),
  ...
  function renderBusinessKnock() {
    ...
    if (elements.businessKnockUnit && elements.businessKnockUnit.options.length !== eligibleUnits.length) {
      elements.businessKnockUnit.replaceChildren(...eligibleUnits.map((unit) => {
        const option = document.createElement("option");
    ...
    if (elements.businessKnockDifficulty) elements.businessKnockDifficulty.value = preset.difficulty;
    ...
    const allOption = elements.businessKnockMode?.querySelector('option[value="all-random"]');
    if (allOption) allOption.textContent = `${poolLabel}から混合`;
    if (elements.businessKnockScope) elements.businessKnockScope.textContent = `${poolLabel}だけの累計。基礎134問の定着判定とは別集計です。`;
    if (elements.businessKnockStats) elements.businessKnockStats.setAttribute("aria-label", `${poolLabel}の累計`);

    const historySummary = BUSINESS_KNOCK?.summarizeHistory?.(businessKnockHistory(preset), new Date()) || {
      attempts: 0,
      accuracy: 0
    };
    const transfer = questions.reduce((counts, question) => {
      const status = BUSINESS_MASTERY?.stateFor(state.practicalDrill?.history?.[question.id] || {}, new Date());
    ...
    const controlsDisabled = Boolean(active) || !businessKnockReady(preset);
    if (elements.businessKnockDifficulty) elements.businessKnockDifficulty.disabled = Boolean(active);
    ...
    if (active) {
      ...
      elements.businessKnockStart.textContent = activeResumeLabel(active);
      elements.businessKnockStatus.textContent = `${active.label}があります。新しいノックで上書きせず、先に再開します。`;
      return;
    }
    if (!businessKnockReady(preset) || !plan) {
      elements.businessKnockStart.disabled = true;
      elements.businessKnockStart.textContent = "ノック問題を読み込めません";
      elements.businessKnockStatus.textContent = `${poolLabel}または選問エンジンを読み込めません。問題の難度を自動で下げず、開始を停止しています。再読込してください。`;
      return;
    }
    ...
    elements.businessKnockStart.textContent = plan.size
      ? `${preset.difficulty === "hard" ? "高難度・" : "基礎・"}${modeLabel}を${plan.size}問ノック開始`
      : `${modeLabel}の対象なし`;
    elements.businessKnockStatus.textContent = plan.size
      ? `${poolLabel}の対象${plan.available}問から${plan.size}問を出題。完了後も同じ条件で次セットへ進めます。${preset.difficulty === "hard" ? "保証協会は含めません。日課20問では、未接触が8問以上あれば少なくとも8問を新規に。残りは誤答・期限を優先します。" : ""}`
      : preset.mode === "weak-due"
        ? "現在、誤答・不安・期限到来はありません。未接触または全範囲を選べます。"
        : preset.mode === "untouched"
          ? `${poolLabel}の対象すべてに接触済みです。弱点・期限、単元、全範囲を選べます。`
          : "この条件で出題できる問題がありません。";
  }
  ...
  function updateBusinessKnockPresetFromControls() {
    ...
    state.practicalDrill.knockPreset = normalizeBusinessKnockPreset({
      difficulty: elements.businessKnockDifficulty?.value,
  ...
  function startBusinessFullScoreSession({
    ...
    const questionsById = new Map(Object.entries(BUSINESS_DRILL_QUESTION_BY_ID));
  ...
  function nextBusinessKnockPresentation(cycleId, previousOffset, preset) {
    const base = `${todayKey()}:knock:${cycleId}`
      .replace(/[^0-9a-z:_-]/gi, "")
      .slice(0, 76);
    for (const suffix of ["a", "b", "c", "d"]) {
      const key = `${base}:${suffix}`;
      const bank = normalizeBusinessKnockPreset(preset).difficulty === "basic" ? BUSINESS_FULLSCORE_BANK : BUSINESS_HARD_BANK;
      const offset = bank.presentQuestion(businessKnockQuestions(preset)[0], key)
        .presentationOffset;
  ...
  function startBusinessKnockSession(requestedSize = 0, options = {}) {
    ...
    const preset = Number.isInteger(requestedSize) && requestedSize > 0
      ? {
          ...defaultPreset,
          ...(daily ? { difficulty: "hard", mode: "all-random", unitId: "", dailyRemainder: requestedSize } : {}),
          size: requestedSize
        }
      : defaultPreset;
    ...
    const presentation = nextBusinessKnockPresentation(cycleId, preset.lastPresentationOffset, preset);
  ...
  function nextGuaranteeSpecialPresentation(cycleId) {
    ...
    const previousKey = state.practicalDrill?.bankId === GUARANTEE_SPECIAL_BANK_ID
      ...
      : "";
    const previousOverrides = state.practicalDrill?.bankId === GUARANTEE_SPECIAL_BANK_ID
    ...
    GUARANTEE_SPECIAL_QUESTION_IDS.forEach((id) => {
      ...
      throw new Error(`保証協会特訓の肢順を更新できませんでした: ${id}`);
    });
    return { key: `${base}:default`, overrides };
  }

  function startGuaranteeSpecialSession(strategy = "smart") {
    const active = activeLearningSession();
    if (active) {
      if (active.kind === "practical" && state.practicalDrill?.bankId === GUARANTEE_SPECIAL_BANK_ID) {
        resumeActiveLearningSession();
    ...
    if (!GUARANTEE_SPECIAL_READY) return;
    const previousState = cloneStateForSync(state);
    const cycleId = createOpaqueId("cycle");
    ...
    state.practicalDrill = {
      ...
      presentationKey: presentation.key,
      presentationOverrides: presentation.overrides,
      planMode: "guarantee",
      stage: "active",
      scope: "business",
  ...
  function setPracticalForecast(confidence) {
    ...
    if (!saveState()) {
      rollbackFailedPracticalDrillMutation(
        previousState,
        "解答前の手応えを保存できませんでした。進捗は変更していません。もう一度選んでください。"
      );
      return;
    }
    ...
    drill.currentAttempt = {
      ...
      diagnosticRecorded: (!correct || predictedWithoutGrounding) &&
        recordPracticalDiagnostic(
          drill.history[question.id],
          question,
          diagnosticTagsForPracticalSelection(question, selected, correct)
        )
    };
    drill.preAnswerConfidence = "";
    practicalGroundingDraft = { questionId: "", axes: new Set() };
  ...
  function advancePracticalDrill() {
    ...
    if (drill.position + 1 < drill.queue.length) {
      ...
      if (drill.bankId === GUARANTEE_SPECIAL_BANK_ID && pending.length) {
        ...
        const eligible = activeStage
          ? pending.filter((id) => {
              const originalIndex = drill.sessionIds.indexOf(id);
              return originalIndex >= 0 && drill.sessionIds.length - originalIndex - 1 >= 3;
            })
          : [];
        const eligibleSet = new Set(eligible);
        const deferred = pending.filter((id) => !eligibleSet.has(id));
        const retryNotBeforeKey = localDateKey(new Date(Date.now() + 24 * 60 * 60 * 1000));
        const retryDecisionAt = new Date().toISOString();
        deferred.forEach((id) => {
          if (drill.history[id]) {
            drill.history[id].retryNotBeforeKey = retryNotBeforeKey;
            drill.history[id].retryNotBeforeAt = retryDecisionAt;
          }
        });
        eligible.forEach((id) => {
          if (drill.history[id]) {
            drill.history[id].retryNotBeforeKey = "";
            drill.history[id].retryNotBeforeAt = retryDecisionAt;
          }
        });
        pending = eligible;
  ...
  function render() {
    ...
    elements.sourceLabel.textContent = isMockMode() && !state.mock.finalized
      ? "時間演習中・根拠条文と解説は終了後に表示"
      : sourceLocator
  ...
  function renderQuestPanel() {
    ...
    [elements.mockCaseButton, elements.mockAButton, elements.mockBButton, elements.mockCButton].forEach((button) => {
      if (!button) return;
      button.disabled = false;
      button.title = "独自問題による演習です。本試験との難易度・得点の等価性は未校正です。";
    ...
    if (isMockMode()) {
      ...
      const activeButton = {
        "case-form-2026-a": elements.mockCaseButton,
  ...
  function renderAdaptiveCoach(question) {
    ...
    if (isMockMode()) {
      elements.coachTitle.textContent = `${mockFormShortLabel()}・演習目標${MOCK_SAFE_TARGET}点`;
  ...
  function mockWeakSourceIds(result) {
    ...
    const legacySourceIds = Array.isArray(question?.sourceQuestionIds)
      ...
      : [];
    if (question?.authoredCase) return [...new Set(legacySourceIds)];
  ...
  function showMockFinished() {
    ...
    const scoreMessage = targetReached
      ? `この演習の目標${safeTarget}点を達成（合否判定ではありません）`
      : (strategyTargetReached
          ? `演習目標${passTarget}点を達成。上位目標${safeTarget}点まであと${safeTarget - score}点`
          : `演習目標${passTarget}点まであと${passTarget - score}点`);
    ...
    showQuizResult((view) => {
      ...
      hero.append(resultElement("span", { text: "得点" }), scoreValue, resultElement("p", { text: scoreMessage }),
        resultElement("p", { className: "mock-evidence-note", text: form.authoredCase
          ? "条件差を判断する独自の事例演習です。本試験の何点相当かは換算できません。初見実力は未見の公式過去問で確認します。"
          : "基礎・既習命題の再構成による練習結果です。出題範囲や難易度に偏りがあり、本試験の安全圏判定には使いません。" }));
      const meta = resultElement("div", { className: "mock-result-meta" });
      [["経過時間（中断を含む）", formatElapsed(state.mock.elapsedMs)], ["誤答", `${wrongResults.length}問`], ["基礎弱点へ登録", `${weakSourceCount}論点`]].forEach(([label, value]) => {
      ...
      priorityPanel.append(
        ...
        resultElement("strong", { text: priority ? `${priority.label} ${priority.correct}/${priority.total} → 目標${priority.target}` : "今回の出題範囲で科目別目標を達成" }),
        resultElement("p", { text: priority ? "誤答の条件・根拠を確認し、翌日以降に別事例で再テストする。" : "未出の論点・例外は未測定です。まだ解いていない公式過去問で時間と科目別得点を確認してください。" })
      );
      ...
      calibration.append(resultElement("strong", { text: "初見実力は公式過去問で確認" }), resultElement("p", { text: "事例実戦Aは個別作問、複合練習A・B・Cは既習命題の再構成です。どちらも公式未見試験の代用ではありません。旧セーブの得点・選択肢順は変更していません。" }), officialButton);
    ...
    $("#mockOfficialExamButton")?.addEventListener("click", () => {
      ...
      if (ledger) ledger.open = true;
      window.requestAnimationFrame(() => {
        const target = elements.officialExamStartButton?.disabled
          ? elements.officialExamId : elements.officialExamStartButton;
        target?.focus({ preventScroll: true });
  ...
  function bindEvents() {
    ...
    elements.mockCaseButton?.addEventListener("click", () => startMock("case-form-2026-a"));
