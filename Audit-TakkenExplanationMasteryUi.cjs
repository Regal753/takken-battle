...
async function runSingleChoice(browser, baseUrl) {
  ...
  assert.equal(result.steps[0].text, result.explain);
  assert.match(result.steps[1].text, /選んだ肢/);
  assert.match(result.steps[1].text, /正解肢/);
  assert.match(result.steps[1].text, /登録|宅建士証|試験/);
  assert.equal(result.steps[2].text, result.trap);
  assert.equal(result.steps[3].text, result.memoryRule);
...
async function runCountQuestion(browser, baseUrl) {
  ...
  const result = await readReasoning(page);
  assert.match(result.steps[1].text, /○はア・ウ・エの3個/);
  assert.match(result.steps[1].text, /答えは「三つ」/);
  assert.equal(result.reasons.length, 4);
  assert.ok(result.reasonLengths.every((length) => length >= 16));
  assert.equal(result.overflow, 0);
  assert.deepEqual(errors, []);
