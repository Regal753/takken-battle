...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() =>
      (document.querySelector("#saveProtectionStatus")?.textContent || "").includes("v5")
    );
    ...
    const readback = await page.evaluate((id) => {
      ...
      return {
        ...
        upgradeRaw: localStorage.getItem(`${id}-before-upgrade-v0-to-v5`) || "",
    ...
    assert.equal(readback.state.stateSchemaVersion, 5);
