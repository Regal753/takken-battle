...
(function attachSubjectSprintBank(root, factory) {
  const supplement = root.TAKKEN_RESTRICTIONS_SUPPLEMENT_BANK || root.window?.TAKKEN_RESTRICTIONS_SUPPLEMENT_BANK || (
    typeof require === "function" ? (() => {
      try { return require("./restrictions-supplement-bank.js"); } catch { return null; }
    })() : null
  );
  const authored = root.TAKKEN_RESTRICTIONS_AUTHORED_BANK || root.window?.TAKKEN_RESTRICTIONS_AUTHORED_BANK || (
    typeof require === "function" ? (() => {
  ...
  const promptStatements = (question) => {
    if (question.format === "単一選択") return question.choices.map(clean);
    const statements = String(question.text || "").split(/\r?\n/)
      .slice(1)
      .map((line) => line.match(/^\s*[アイウエ]\s+(.+)$/)?.[1])
      .filter(Boolean)
      .map(clean);
    if (statements.length !== 4) throw new Error(`${question.id}: non-single prompt must contain ア〜エ statements`);
    return statements;
  };
  const baseFacts = (question) => {
    if (Array.isArray(question.sourceFacts) && question.sourceFacts.length === 4) return Object.freeze(question.sourceFacts.map(Object.freeze));
    ...
    return Object.freeze(explanations.map((explanation, index) => {
      ...
      return Object.freeze({
        ...
        sourceUrls: Object.freeze((Array.isArray(question.sourceUrls) ? question.sourceUrls : [question.sourceUrl]).map(clean).filter(Boolean)),
  ...
  const baseTopicAliases = {
    ...
    rs001:["開発工事の完了検査・公告",["development-permit","completion-notice"]],rs002:["市街化調整区域の建築許可",["urbanization-control-area","building-permit"]],rs003:["令和7年建築確認対象見直し",["building-confirmation","current-law"]],rs004:["木造戸建ての大規模修繕",["building-confirmation","major-renovation","current-law"]],rs005:["国土法の国籍等届出事項",["national-land","notification-items","current-law"]],rs006:["事後届出後の勧告・公表",["national-land","recommendation"]],rs007:["市街化区域内の農地転用届出",["agricultural-land","conversion","urbanization-area"]],rs008:["農地取得の効力・相続・一時転用",["agricultural-land","inheritance","temporary-conversion"]],rs009:["換地処分公告後の権利移転",["land-readjustment","replot-disposition"]],rs010:["減歩・清算金",["land-readjustment","land-reduction","settlement-money"]],rs011:["盛土規制法の対象・区域",["embankment","regulated-area"]],rs012:["盛土許可申請前後の義務",["embankment","permit-process"]],rs013:["遺跡発見時の届出",["cultural-property","discovery-notice"]],rs014:["道路占用許可と道路使用許可",["road-occupation","road-use-permit"]],rs015:["開発許可の適用除外",["development-permit","permit-exemption","public-actor"]],rs016:["市街化調整区域の立地基準",["development-permit","urbanization-control-area","location-standard"]],rs017:["特殊建築物への用途変更",["building-confirmation","change-of-use","boundary-200"]],rs018:["前面道路による容積率計算",["floor-area-building-coverage","road-width","calculation"]],rs019:["北側斜線制限",["height-restriction","north-side-setback"]],rs020:["一団の土地と面積合算",["national-land","aggregate-land","post-contract-notice"]],rs021:["農地法3条・4条・5条の仕分け",["agricultural-land","conversion","article-routing"]],rs022:["保留地の帰属時点",["land-readjustment","reserved-land","replot-disposition"]],
  ...
  const questions = definitions.map(([id, baseId, sectionId, tag, diagnosticTags, variantOffset]) => {
    const source = sourceQuestions[baseId];
    ...
    return Object.freeze({id,masteryId:id,sourceQuestionId:baseId,sourceAnchor:clean(source.sourceAnchor||source.sourceRef),sectionId,tag,diagnosticTags,variantOffset,format:source.format,text:source.text,choices:source.choices,answer:source.answer,explain:source.explain,trap:source.trap,memoryRule:source.memoryRule,groundingFrame:source.groundingFrame||null,choiceExplanations:source.choiceExplanations,statementExplanations:source.choiceExplanations,sourceFacts:baseFacts(source),sourceRef:source.sourceRef,sourceLocator:source.sourceLocator,sourceUrl:source.sourceUrl,sourceUrls:Object.freeze((Array.isArray(source.sourceUrls)?source.sourceUrls:[source.sourceUrl]).map(clean).filter(Boolean)),legalBaseline:source.legalBaseline,verifiedAt:source.verifiedAt,...(source.authoredCase ? {authoredCase:true,formatKey:source.formatKey,premise:source.premise,stem:source.stem,targetLevel:source.targetLevel,displayModel:source.displayModel,legalSources:source.legalSources,noveltyNote:source.noveltyNote,changeNote:source.changeNote,qualityVersion:source.qualityVersion,choiceOriginIndexes:source.choiceOriginIndexes} : {})});
  });
  ...
  function presentQuestion(questionOrId,presentationKey="") { const question=stableQuestion(questionOrId); const offset=(question.variantOffset+(stableHash(presentationKey||question.id)%4))%4; const order=Object.freeze(rotate([0,1,2,3],offset)); const isSingle=question.format==="単一選択"; const explanations=Object.freeze(isSingle?order.map((index)=>question.statementExplanations[index]):[...question.statementExplanations]); const displayModel=question.displayModel ? Object.freeze({...question.displayModel,choiceBlocks:Object.freeze(isSingle?order.map((index)=>question.displayModel.choiceBlocks[index]):[...question.displayModel.choiceBlocks]),items:Object.freeze([...question.displayModel.items])}) : undefined; return Object.freeze({...question,choices:Object.freeze(order.map((index)=>question.choices[index])),choiceExplanations:explanations,statementExplanations:explanations,sourceFacts:Object.freeze(isSingle?order.map((index)=>question.sourceFacts[index]):[...question.sourceFacts]),...(displayModel?{displayModel}:{}),answer:order.indexOf(question.answer),presentationKey:clean(presentationKey)||question.id,presentationOrder:order,presentationOffset:offset}); }
  function diversify(questionIds, recentQuestionIds = []) {
    const ids = [...new Set((Array.isArray(questionIds) ? questionIds : []).map(String))]
      .filter((id) => Boolean(questionsById[id]));
    const recent = (Array.isArray(recentQuestionIds) ? recentQuestionIds : [])
      .map((id) => questionsById[String(id)])
      .filter(Boolean)
      .slice(-2);
    const pending = ids.map((id, index) => ({ question: questionsById[id], index }));
    const result = [];
    while (pending.length) {
      let bestIndex = 0;
      let bestScore = null;
      const anchorCounts = pending.reduce((counts, item) => {
        counts.set(item.question.sourceAnchor, (counts.get(item.question.sourceAnchor) || 0) + 1);
        return counts;
      }, new Map());
      const completionRisk = new Map();
      pending.forEach(({ question, index }, candidateIndex) => {
        const anchorPenalty = recent.reduce((sum, previous, recentIndex) =>
          sum + (question.sourceAnchor === previous.sourceAnchor ? (recentIndex === recent.length - 1 ? 2 : 1) : 0), 0);
        // Keep the caller's mastery priority unless it would leave a same-law
        // tail that cannot be separated. Adding large authored packs makes a
        // purely greedy priority order consume the smaller subjects too early.
        const nextRecent = [...recent, question].slice(-2);
        const remaining = pending.length - 1;
        let stranded = 0;
        for (const [anchor, count] of anchorCounts) {
          const left = count - Number(anchor === question.sourceAnchor);
          const firstSlot = nextRecent.at(-1)?.sourceAnchor === anchor ? 2
            : nextRecent.at(-2)?.sourceAnchor === anchor ? 1 : 0;
          const capacity = remaining > firstSlot ? Math.floor((remaining - 1 - firstSlot) / 3) + 1 : 0;
          stranded += Math.max(0, left - capacity);
        }
        if (!completionRisk.has(question.sourceAnchor)) {
          const counts = new Map(anchorCounts);
          counts.set(question.sourceAnchor, counts.get(question.sourceAnchor) - 1);
          const cooldown = nextRecent.map(item => item.sourceAnchor);
          let impossible = false;
          for (let step = 0; step < remaining; step++) {
            const candidates = [...counts].filter(([anchor, count]) => count > 0 && !cooldown.includes(anchor))
              .sort((left, right) => right[1] - left[1]);
            if (!candidates.length) { impossible = true; break; }
            const [anchor, count] = candidates[0];
            counts.set(anchor, count - 1);
            cooldown.push(anchor);
            if (cooldown.length > 2) cooldown.shift();
          }
          completionRisk.set(question.sourceAnchor, Number(impossible));
        }
        stranded += completionRisk.get(question.sourceAnchor);
        const score = [anchorPenalty, stranded, index];
        if (!bestScore || score[0] < bestScore[0] || (score[0] === bestScore[0] &&
            (score[1] < bestScore[1] || (score[1] === bestScore[1] && score[2] < bestScore[2])))) {
          bestScore = score;
          bestIndex = candidateIndex;
        }
      });
      const [{ question }] = pending.splice(bestIndex, 1);
  ...
  return Object.freeze({VERSION,LEGAL_BASELINE,QUESTIONS:Object.freeze(questions),QUESTIONS_BY_ID:questionsById,COVERAGE:coverage,presentQuestion,stableQuestion,diversify});
});
